...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // AuthZ: require a valid Supabase user JWT, and only allow emailing the authenticated user's email.
    ...
    if (String(to).toLowerCase() !== authedEmail) {
      return new Response(
        JSON.stringify({ success: false, error: 'Forbidden: can only email the authenticated user' }),
